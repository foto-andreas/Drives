...
- **Scan-Flow:** Der Scan-Screen nutzt Geolocation und Foto-Upload. Start- und Ziel-Scans werden per OCR (KM-Stand) und Reverse-Geocoding (Adresse) ergänzt; der Grund kann ausgewählt werden (Default `sonstiges`) und die Fahrt kann übernommen werden.
